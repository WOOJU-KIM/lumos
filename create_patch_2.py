...
'''    min_date = master['Datetime'].min()
    max_date = pd.to_datetime("2026-09-14 00:00:00")
    test_start = pd.to_datetime("2026-08-31 00:00:00")'''
)
...
save_code = '''
    ...
    if trades:
        ...
        pd.DataFrame(columns=['entry_dt', 'capital']).to_csv('data/wfa_upro_trades.csv', index=False)
        print("✅ No trades found. Saved empty CSVs.")
'''
...
# Fix the internal 'import pandas as pd' if any
code = code.replace("import pandas as pd\\n    if trades:", "if trades:")

with open('temp_run_custom_week_2.py', 'w', encoding='utf-8') as f:
    f.write(code)
