...
def run_weekly_rolling_walk_forward():
    ...
    upro_15m = lake.load_candles("UPRO", "15m")
    spxu_15m = lake.load_candles("SPXU", "15m")
    spy_60m = lake.load_candles("SPY", "60m")
    nvda_15m = lake.load_candles("NVDA", "15m")
    soxx_15m = lake.load_candles("SOXX", "15m")
    spy_15m  = lake.load_candles("SPY", "15m")
    vixy_15m = lake.load_candles("VIXY", "15m")
    ief_15m  = lake.load_candles("IEF", "15m")

    # 60m SPY EMA
    spy_60m['ema_filter'] = spy_60m['Close'].ewm(span=config.SPY_EMA_PERIOD, adjust=False).mean()
    ...
    upro_feat = ml_engine.extract_features(upro_15m)
    labels = ml_engine.compute_triple_barrier_labels(upro_feat)

    # 타겟 라벨 매핑: 1 -> 2 (Long TP), -1 -> 0 (Short TP), 0 -> 1 (Neutral)
    target_series = labels.map({1: 2, -1: 0, 0: 1}).fillna(1).astype(int)
    upro_feat['target'] = target_series

    # ISO 주차(Year-Week) 태깅
    upro_feat['datetime_dt'] = pd.to_datetime(upro_feat['datetime'])
    upro_feat['date_str'] = upro_feat['datetime_dt'].dt.strftime('%Y-%m-%d')
    upro_feat['time_str'] = upro_feat['datetime_dt'].dt.strftime('%H:%M')
    upro_feat['week_id'] = upro_feat['datetime_dt'].dt.isocalendar().year.astype(str) + '-' + upro_feat['datetime_dt'].dt.isocalendar().week.astype(str).str.zfill(2)

    # 크로스에셋 딕셔너리 고속 캐싱
    nvda_map = nvda_15m.set_index('datetime')['Close'].to_dict()
    soxx_map = soxx_15m.set_index('datetime')['Close'].to_dict()
    spy_map  = spy_15m.set_index('datetime')['Close'].to_dict()
    vix_map  = vixy_15m.set_index('datetime')['Close'].to_dict()
    ief_map  = ief_15m.set_index('datetime')['Close'].to_dict()
    spxu_dict = spxu_15m.set_index('datetime').to_dict(orient='index')
    ...
    feature_cols = [c for c in upro_feat.columns if c not in exclude_cols and pd.api.types.is_numeric_dtype(upro_feat[c])]
    print(f"📊 학습 피처 수: {len(feature_cols)}개 피처")

    # 주차 목록 정렬
    unique_weeks = sorted(upro_feat['week_id'].unique())
    ...
    for w_idx in range(test_start_idx, len(unique_weeks)):
        ...
        train_mask = upro_feat['week_id'].isin(train_weeks)
        X_train = upro_feat.loc[train_mask, feature_cols].fillna(0.0)
        y_train = upro_feat.loc[train_mask, 'target']
        ...
        test_mask = upro_feat['week_id'] == cur_test_week
        test_df = upro_feat[test_mask].copy()
        if test_df.empty:
            continue
        ...
        for d_str in test_dates:
            ...
            for idx, row in day_bars.iterrows():
                ...
                cur_upro_close = float(row['Close'])
                cur_upro_high = float(row['High'])
                cur_upro_low = float(row['Low'])

                spxu_row = spxu_dict.get(curr_dt)
                cur_spxu_close = float(spxu_row['Close']) if spxu_row else 0.0
                cur_spxu_high = float(spxu_row['High']) if spxu_row else 0.0
                cur_spxu_low = float(spxu_row['Low']) if spxu_row else 0.0
                ...
                if active_pos is not None:
                    ...
                    cur_close = cur_upro_close if sym == 'UPRO' else cur_spxu_close
                    cur_high = cur_upro_high if sym == 'UPRO' else cur_spxu_high
                    cur_low = cur_upro_low if sym == 'UPRO' else cur_spxu_low
                ...
                if active_pos is None and time_str < config.PHASE_MAIN_END:
                    past_spy_60 = spy_60m[spy_60m['datetime'] <= curr_dt]
                    spy_60m_bull = True
                    spy_60m_bear = True
                    if len(past_spy_60) >= 20:
                        q_c = past_spy_60['Close'].iloc[-1]
                        q_ema = past_spy_60['ema_filter'].iloc[-1]
                        spy_60m_bull = (q_c >= q_ema * 0.998)
                        spy_60m_bear = (q_c <= q_ema * 1.002)
                    ...
                    if cur_bar_idx >= 5 and n_px and sx_px and q_px:
                        ...
                        spy_r  = (q_px / p_q - 1.0) if p_q else 0.0
                        vix_r  = (v_px / p_v - 1.0) if (v_px and p_v) else 0.0
                        ief_r  = (i_px / p_i - 1.0) if (i_px and p_i) else 0.0
                        tnx_proxy_ret = -ief_r

                        upro_r = (cur_upro_close / day_bars.iloc[cur_bar_idx - 5]['Close']) - 1.0
                        ...
                        if sig_code > 0:
                            ...
                            cross_dir = "SHORT_SPXU"

                    if dir_gbdt == "LONG_UPRO" and conf_gbdt >= config.GBDT_CONFIDENCE_THRESHOLD and spy_60m_bull and cross_dir != "SHORT_SPXU":
                        active_pos = {
                            'sym': 'UPRO',
                            'entry_px': cur_upro_close,
                            'entry_dt': curr_dt,
                            'bars': 0,
                            'peak_high': cur_upro_close
                        }
                    elif dir_gbdt == "SHORT_SPXU" and conf_gbdt >= config.GBDT_CONFIDENCE_THRESHOLD and spy_60m_bear and cross_dir != "LONG_UPRO":
                        if cur_spxu_close > 0:
                            active_pos = {
                                'sym': 'SPXU',
                                'entry_px': cur_spxu_close,
                                'entry_dt': curr_dt,
                                'bars': 0,
                                'peak_high': cur_spxu_close
                            }
    ...
    def calc_stats(sub_df, start_cap):
        ...
        return {
            ...
            'end_cap': int(end_cap),
            'upro_trades': len(sub_df[sub_df['symbol'] == 'UPRO']),
            'spxu_trades': len(sub_df[sub_df['symbol'] == 'SPXU'])
        }
    ...
    for y in years:
        ...
        yearly_rows.append({
            ...
            'pf': float(st['pf']),
            'upro_cnt': int(st['upro_trades']),
            'spxu_cnt': int(st['spxu_trades'])
        })
    ...
    print(f"• 총 거래 횟수: {total_stats['trades']}회 (UPRO: {total_stats['upro_trades']}회 / SPXU: {total_stats['spxu_trades']}회)")
    ...
    print("=" * 115)
    print(f"{'년도':<6} | {'시작 잔고':<16} | {'기말 잔고':<16} | {'연간 손익':<16} | {'수익률':<9} | {'거래수':<6} | {'승률':<7} | {'PF':<6} | {'UPRO/SPXU':<10}")
    print("-" * 115)
    for r in yearly_rows:
        print(f"{r['year']:<6} | {r['start_cap']:>14,}원 | {r['end_cap']:>14,}원 | {r['pnl']:>+14,}원 | {r['ret_pct']:>+7.1f}% | {r['trades']:>4}회 | {r['win_rate']:>5.1f}% | {r['pf']:>5.2f} | {r['upro_cnt']:>2}/{r['spxu_cnt']:<2}회")
    print("=" * 115)

    print("\n" + "=" * 115)
    print("🗓️ [3. 월별 상세 결산 (천만 원 시작 복리 운용 잔고, 49개월)]")
    print("=" * 115)
    print(f"{'연월':<7} | {'시작 잔고':<16} | {'기말 잔고':<16} | {'월간 손익':<16} | {'수익률':<9} | {'거래수':<6} | {'승률':<7} | {'PF':<6} | {'UPRO/SPXU':<10}")
    print("-" * 115)
    for r in monthly_rows:
        print(f"{r['month']:<7} | {r['start_cap']:>14,}원 | {r['end_cap']:>14,}원 | {r['pnl']:>+14,}원 | {r['ret_pct']:>+7.2f}% | {r['trades']:>4}회 | {r['win_rate']:>5.1f}% | {r['pf']:>5.2f} | {r['upro_cnt']:>2}/{r['spxu_cnt']:<2}회")
    print("=" * 115)

    # 저장
    clean_summary = sanitize_for_json(result_summary)
    output_path = DATA_DIR / "backtest_upro_summary.json"
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(clean_summary, f, ensure_ascii=False, indent=2)

    df_trades.to_csv(DATA_DIR / "backtest_upro_trades.csv", index=False, encoding="utf-8-sig")
