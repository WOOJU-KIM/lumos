...
def run_weekly_rolling_walk_forward():
    ...
    tqqq_15m = lake.load_candles("SOXL", "15m")
    sqqq_15m = lake.load_candles("SOXS", "15m")
    qqq_60m = lake.load_candles("SOXX", "60m")
        ...
        for d_str in test_dates:
            ...
            for idx, row in day_bars.iterrows():
                ...
                if active_pos is not None:
                    ...
                    cur_close = cur_tqqq_close if sym == 'SOXL' else cur_sqqq_close
                    cur_high = cur_tqqq_high if sym == 'SOXL' else cur_sqqq_high
                    cur_low = cur_tqqq_low if sym == 'SOXL' else cur_sqqq_low
                ...
                if active_pos is None and time_str < config.PHASE_MAIN_END:
                    ...
                    if dir_gbdt == "LONG_SOXL" and conf_gbdt >= config.GBDT_CONFIDENCE_THRESHOLD and qqq_60m_bull and cross_dir != "SHORT_SOXS":
                        active_pos = {
                            'sym': 'SOXL',
                    ...
                    elif dir_gbdt == "SHORT_SOXS" and conf_gbdt >= config.GBDT_CONFIDENCE_THRESHOLD and qqq_60m_bear and cross_dir != "LONG_SOXL":
                        if cur_sqqq_close > 0:
                            active_pos = {
                                'sym': 'SOXS',
    ...
    output_path = DATA_DIR / "backtest_summary_SOXL.json"
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(clean_summary, f, ensure_ascii=False, indent=2)

    df_trades.to_csv(DATA_DIR / "backtest_trades_SOXL.csv", index=False, encoding="utf-8-sig")
