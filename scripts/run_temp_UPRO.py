...
def run_weekly_rolling_walk_forward():
    ...
    tqqq_15m = lake.load_candles("UPRO", "15m")
    sqqq_15m = lake.load_candles("SPXU", "15m")
    qqq_60m = lake.load_candles("SPY", "60m")
        ...
        for d_str in test_dates:
            ...
            for idx, row in day_bars.iterrows():
                ...
                if active_pos is not None:
                    ...
                    cur_close = cur_tqqq_close if sym == 'UPRO' else cur_sqqq_close
                    cur_high = cur_tqqq_high if sym == 'UPRO' else cur_sqqq_high
                    cur_low = cur_tqqq_low if sym == 'UPRO' else cur_sqqq_low
                ...
                if active_pos is None and time_str < config.PHASE_MAIN_END:
                    ...
                    if dir_gbdt == "LONG_UPRO" and conf_gbdt >= config.GBDT_CONFIDENCE_THRESHOLD and qqq_60m_bull and cross_dir != "SHORT_SPXU":
                        active_pos = {
                            'sym': 'UPRO',
                    ...
                    elif dir_gbdt == "SHORT_SPXU" and conf_gbdt >= config.GBDT_CONFIDENCE_THRESHOLD and qqq_60m_bear and cross_dir != "LONG_UPRO":
                        if cur_sqqq_close > 0:
                            active_pos = {
                                'sym': 'SPXU',
    ...
    output_path = DATA_DIR / "backtest_summary_UPRO.json"
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(clean_summary, f, ensure_ascii=False, indent=2)

    df_trades.to_csv(DATA_DIR / "backtest_trades_UPRO.csv", index=False, encoding="utf-8-sig")
